...
export async function GET(req: NextRequest) {
  ...
  if (!key) return NextResponse.json({ location: null }, { status: 500 });
  ...
  try {
    ...
    const data = (await response.json()) as KakaoCoordResponse;

    const region =
      data.documents?.find((document) => document.region_type === 'H') ??
    ...
    const sidoKey = Object.keys(SIDO_CODE_MAP).find((keyName) => sidoName.startsWith(keyName));
    ...
    return NextResponse.json({
      location: {
        ...
        sigunguName: region.region_2depth_name,
        lat: parseFloat(lat),
        lng: parseFloat(lng),
      },
    });
  } catch (error) {
    console.error('[coord2region]', error);
