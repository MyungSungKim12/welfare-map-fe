...
**메인 UI / 화면 라우트**
- `AI 지역생활 복지 레이더` → `AI 복지 컨시어지` 컨셉으로 메인 UI 재구성
- 데스크톱 / 모바일 반응형 Hero, 필터, 지도, 리스트, 푸터 화면
- 4개 라우트: `/` 랜딩, `/login` 소셜 로그인 UI (버튼만), `/onboarding` 사용자 프로필 입력, `/search` AI 검색 워크스페이스
- `/search` 좌측 메뉴: AI 검색 / 내 맞춤 혜택 / 지도·주변기관 / 저장한 혜택 / 검색 기록 / 마이페이지
- 검색 실행 후 결과 집중 모드 (좌측 프롬프트 카드 접힘 → 중앙 결과 리스트)

**위치·지도**
- GPS 기반 현재 위치 감지, 수동 지역 선택
- 카카오맵 지도 화면, 위치 기반 복지 리스트 조회, 지도-리스트 데이터 공유
- Kakao Local 키워드 검색 기반 주변 복지기관 마커 (복지관/주민센터/보건소/노인·장애인복지관/어린이집)

**통합 검색 (다중 소스)**
- 자연어 쿼리 → **Gemini AI intent 파싱** (생애주기·관심분야·지역·긴급도 자동 추출)
- **Gemini google_search grounding 웹 검색** — 공식 정부·지자체 도메인 우선
- 복지로 지자체·전국 복지 API
- **서울 열린데이터광장 3종 통합** (1인가구 프로그램, 강동구 어르신 시설, 은평구 시설대관)
- 통합 검색 라우터 `/api/benefits/search` — 소스 병렬 호출·정규화·dedup·랭킹
- 검색 결과 우측 패널에 AI 분석 결과 chip 카드 (생애주기·관심분야·지역·긴급도·키워드·confidence)

**사용자 데이터**
- 사용자 프로필 입력 (localStorage MVP) + 프로필 기반 프롬프트 추천
- 복지 저장 / 북마크 (localStorage MVP, Navbar 카운트 배지 + 저장한 복지 섹션)
- 검색 기록 localStorage 저장 + 좌측 메뉴에서 재실행

**집계·데이터**
- 실제 API 결과 기반 Hero 요약 / 알림 / 추천 후보 / 통계 섹션
- Supabase 캐시 연동 코드와 DB migration 실제 적용 (활성 검증 완료)
- **인기 복지 집계** (BE popular 도메인) — view/click/save 카운트 upsert + LEFT JOIN 응답
- FE 카드 인터랙션 → BE 카운터 이벤트 자동 발사 (dedup ref set)
- 마감 임박 / 신규 / 만료 배지

### 아직 더미 또는 미완성인 기능

- **실제 소셜 OAuth 연결** (Kakao/Naver 버튼 UI만, 콜백/JWT 미연결)
- **북마크 / 사용자 프로필 Supabase 동기화** (현재는 localStorage MVP)
- **planned 데이터 소스** (환경변수 대기 상태로 자동 활성화 예정): 온라인청년센터, 정부24, 고용24, 보건복지, LH/SH 주거, 소상공인
- **배치 작업** (인기 복지 일별 집계, 마감 임박 알림 등)
- **AI 추천 이유 / 신청 체크리스트 자동 생성** (Gemini 재활용 여지)
- **지역생활 행사 / 문화 / 날씨 API** 연동
- BE 인증 도메인·사용자 도메인·배치 도메인 (현재는 popular 만)
...
| Supabase Data API / 캐시 | 연동됨 (DB 적용 + Data API 200 확인) | 기본 복지 조회 결과 캐싱 |
| **Gemini AI intent 파싱** | 연동됨 | 자연어 쿼리에서 생애주기·관심분야·지역·긴급도·키워드 구조화 |
| **Gemini google_search grounding** | 연동됨 | 공식 정부·지자체 도메인 우선 웹 검색 통합 |
| **서울 열린데이터광장 (3종)** | 연동됨 | 1인가구 프로그램·강동구 어르신 시설·은평구 시설대관 |
| 온라인청년센터 청년정책 | 코드 준비 / API 키 대기 | 청년정책·전세대출·취업 통합 |
| 정부24 혜택알리미 | 코드 준비 예정 / API 키 대기 | 정부 통합 혜택 |
| 고용24 | 코드 준비 예정 / API 키 대기 | 일자리·직업훈련 |
| 보건복지 API | 코드 준비 예정 / API 키 대기 | 건강검진·의료비 |
| LH/SH 주거 | 코드 준비 예정 / API 키 대기 | 임대주택·월세지원 |
| 소상공인/중기부 | 코드 준비 예정 / API 키 대기 | 창업·성장·폐업 지원 |
...
- **소셜 OAuth 미연결**: `/login` 은 버튼 UI 만 있고 실제 Kakao/Naver 인증 파이프라인 없음
- **북마크 / 프로필 서버 저장 미연결**: localStorage MVP 유지, 로그인 도입 후 Supabase 동기화 필요
- **planned 데이터 소스 활성화**: 청년정책·정부24·고용24·보건복지·LH·소상공인 API 활용신청 후 어댑터 완성
- **BE 도메인 확장 필요**: 현재는 popular 만. 인증·사용자·배치·알림 도메인 미구현
- 필터가 적용된 복지 API 조회는 아직 외부 API를 직접 호출 (캐시 우회)
- BE DB 비밀번호는 회전됐지만 이전 history 에는 노출된 상태 (Git history rewrite 는 하지 않음)
- Gemini API 호출 비용 관리 필요 (`ai-web-search` 는 매 검색마다 호출 → 캐싱·rate limit 추가 여지)
- `npm audit` 잔여 2건은 Next.js 내부 postcss 전이 의존성으로 `--force` 시 Next 9.x 다운그레이드 → 안전 패치 한계
...
### 최신 다음 작업 우선순위 (2026.07.02 기준)

이전 계획 대비 진행 상황:
- AI intent 파싱 ✅
- AI 웹 검색 grounding ✅
- 서울 오픈데이터 커넥터 ✅
- 청년정책 어댑터 코드 준비 (API 키 대기)
- 좌측 메뉴 5개 서브 화면 ✅

**남은 우선순위**:

1. **소셜 OAuth 실 연결** — Kakao/Naver OAuth 콜백 + JWT + FE 로그인 상태 훅. BE `user` 도메인. Supabase `users` 연동.
2. **planned 데이터 소스 활성화** — 사용자가 공공데이터포털에서 확보한 API 키 순으로 어댑터 완성 (청년정책 → 고용24 → 정부24 → 보건복지 순 권장).
3. **AI 추천 이유 / 체크리스트 자동 생성** — 검색 결과 카드마다 Gemini 로 "왜 추천됐는지" + 신청 준비물 생성.
4. **북마크 / 프로필 Supabase 동기화** — OAuth 완성 이후 localStorage → `saved_services` / `user_profiles`.
5. **AI 웹 검색 캐싱 + rate limit** — Gemini 비용 관리. 동일 쿼리 5분 캐시, 사용자별 rate.
6. **지도/주변기관 화면 실동작** — 좌측 메뉴 wireframe 을 기존 `Map` + `useNearbyFacilities` 로 연결.
7. **지역 행사 / 날씨 API** — 4순위 완성.
